...
//    }
    ...
    public OFD read(File file) throws IOException {
        FileManager fileManager = readZip(file);
        return read(fileManager);
    }

    public OFD read(byte[] file) throws IOException {
        FileManager fileManager = readZip(file);
        return read(fileManager);
    }

    private OFD read(FileManager fileManager){
        try {
            OFD ofd = new OFD();
    ...
    private FileManager readZip(byte[] file) throws IOException {
        ...
        try (InputStream inputStream = new ByteArrayInputStream(file);
            ...
            while ((entry=zipInputStream.getNextEntry())!=null){
                if (!entry.isDirectory()) {
                    fileManager.write("/" + entry.getName(), zipInputStream);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private FileManager readZip(File file) throws IOException {
        FileManager fileManager = new MemoryFileManager();
